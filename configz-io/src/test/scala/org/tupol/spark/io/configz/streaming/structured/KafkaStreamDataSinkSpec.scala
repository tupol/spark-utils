...
class KafkaStreamDataSinkSpec extends AnyFunSuite with Matchers with Eventually with SharedSparkSession
  ...
  test("Saving the input data as Json results in the same data") {
    ...
    val genericConfig = GenericStreamDataSinkConfiguration(Kafka, Map(), Some("testQuery"),
