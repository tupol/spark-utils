...
import org.tupol.spark.io.streaming.structured.{
  ...
  FormatAwareStreamingSinkConfiguration,
...
object readers {
  ...
  implicit val KafkaSubscriptionReader: ConfigReader[KafkaSubscription] =
    ConfigReader
      .fromCursor[KafkaSubscription] { cur =>
        for {
      ...
      .ensure(
        ...
        subscription =>
          s"The subscription.type '${subscription.subscriptionType}' is not supported. " +
    ...
    ConfigReader.fromCursor[FileStreamDataSourceConfiguration] { cur =>
      for {
        ...
        _ <- if (FormatType.AcceptableFileFormats.contains(format)) Right(())
              ...
              formatCur.failed(new FailureReason {
  ...
  implicit val GenericStreamDataSourceConfigurationReader: ConfigReader[GenericStreamDataSourceConfiguration] = {
    import org.tupol.spark.io.pureconf.readers._
    ConfigReader
      .forProduct3[GenericStreamDataSourceConfiguration, FormatType, Option[Map[String, String]], Option[StructType]](
        "format",
        "options",
        "schema"
      )(GenericStreamDataSourceConfiguration(_, _, _))
      .ensure(
        conf => FormatType.AcceptableStreamingFormats.contains(conf.format),
        conf =>
  ...
  implicit val GenericStreamDataSinkConfigurationReader: ConfigReader[GenericStreamDataSinkConfiguration] = {
    import org.tupol.spark.io.pureconf.readers._
    ConfigReader
      .forProduct6[GenericStreamDataSinkConfiguration, FormatType, Option[Map[String, String]], Option[String], Option[
        Trigger
      ], Option[PartitionsConfiguration], Option[String]](
        "format",
        "options",
        "queryName",
        "trigger",
        "partition",
        "outputMode"
      )(GenericStreamDataSinkConfiguration(_, _, _, _, _, _))
      .ensure(
        conf => FormatType.AcceptableStreamingFormats.contains(conf.format),
        conf =>
  ...
  implicit val KafkaStreamDataSourceConfigurationReader: ConfigReader[KafkaStreamDataSourceConfiguration] = {
    ...
    ConfigReader.fromCursor[KafkaStreamDataSourceConfiguration] { cur =>
      for {
        ...
        kafkaConsumerPollTimeoutMs <- ConfigReader[Option[Long]]
                                       .from(objCur.atKeyOrUndefined("kafkaConsumerPollTimeoutMs"))
        fetchOffsetNumRetries <- ConfigReader[Option[Int]].from(objCur.atKeyOrUndefined("fetchOffsetNumRetries"))
        fetchOffsetRetryIntervalMs <- ConfigReader[Option[Long]]
                                       .from(objCur.atKeyOrUndefined("fetchOffsetRetryIntervalMs"))
        ...
        fetchOffsetNumRetries,
        fetchOffsetRetryIntervalMs,
