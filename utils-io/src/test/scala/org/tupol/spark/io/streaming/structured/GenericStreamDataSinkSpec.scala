...
class GenericStreamDataSinkSpec
    extends AnyFunSuite
    with Matchers
    with Eventually
  ...
  test("Saving the input data as Json results in the same data") {
    ...
    val TestOptions =
      Map("kafka.bootstrap.servers" -> s":${config.kafkaPort}", "topic" -> topic, "checkpointLocation" -> testPath1)
  ...
  test("Fail gracefully") {
    ...
    val TestOptions =
      Map("kafkaBootstrapServers" -> s"unknown_host:0000000", "topic" -> topic, "checkpointLocation" -> "")
